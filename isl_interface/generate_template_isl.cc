...
std::ostream& operator<<(std::ostream& os, const BaseKind& base) {
  if (base.children.size() == 2) {
    return os << "Pair<" << base.children[0] << "," << base.children[1] << ">";
  } else {
    return os << base.name;
  }
}
...
static Signature<Kind> apply_domain() {
  return {{"Range2", "Range"}, {{"Domain", "Range"}, {"Domain", "Range2"}}};
}
...
static const std::unordered_map<std::string, std::vector<Signature<Kind>>>
    staticSignatures{
        ...
        {"universe", {modify_params_unary(), modify_set_unary(), modify_map_unary()}},
        {"zero", {modify_set_unary()}},
...
static const std::unordered_map<std::string, std::vector<Constructor>>
    constructors{
        ...
        {"aff_list", {{{"aff"}, {modify_set_unary(), modify_map_unary()}}}},
...
template <typename T>
static void printTemplateList(const std::vector<T> list, const std::string& qualifier) {
  std::cout << "<";
  for (unsigned i = 0; i < list.size(); ++i) {
    if (i > 0) {
      std::cout << ", ";
    }
    std::cout << qualifier << list[i];
  }
  std::cout << ">";
}
...
static void printExtraTemplate(
  ...
  for (size_t i = 0; i < match.size(); ++i) {
    if (match[i] != classKind[i]) {
      std::stringstream ss;
      ss << "= std::enable_if<std::is_same<" << match[i] << ", " << classKind[i]
         << ">::value>";
...
static void printType(const Type& type, const Kind& kind) {
...
static void printReturnType(
    const Signature<Kind>& signature,
    const Method& method) {
  printType(method.signature.returnType, signature.returnType);
}
...
static void printMethod(
    ...
    const Method& method,
    bool isStatic = false) {
  printExtraTemplate(classKind, signature, isStatic);
  if (isStatic) {
    std::cout << "static ";
  }
  std::cout << "inline ";
  printReturnType(signature, method);
  std::cout << " ";
  ...
  for (size_t i = 0; i < method.signature.argTypes.size(); ++i) {
    ...
    if (classes.count(type) == 0) {
      ...
      printType(type, signature.argTypes[j++]);
    }
    std::cout << "& arg" << i;
  }
  std::cout << ")";
...
static void printForeach(
    const std::string& base,
    const Kind& classKind, const Method& method) {
...
static void printMethods(
  ...
  for (auto method : methods) {
	...
	if (matches(classKind, signature, method)) {
          printMethod(base, classKind, signature, method);
        }
      }
      ...
      for (const auto& signature :
        ...
        if (signature.returnType.size() == classKind.size()) {
          printMethod(base, classKind, signature, method, true);
        }
      }
	...
	if (matches(classKind, signature, method)) {
      ...
      for (auto returnKind : classes.at(method.signature.returnType).kinds) {
        if (classKind.size() == 2 && returnKind.size() == 2) {
          printTo(modify_map_unary(), method);
        }
      }
    } else if (
        method.name == "#as" &&
...
static void printConstructor(
  ...
  for (size_t i = 0; i < argTypes.size(); ++i) {
    ...
    std::cout << "const ";
    printType(argTypes[i], signature.argTypes[i]);
    std::cout << "& arg" << i;
  }
  std::cout << ") : " << base << "(";
  for (size_t i = 0; i < argTypes.size(); ++i) {
...
static void printDefinition(
  ...
  if (constructors.count(base) != 0) {
	...
	if (classKind.size() == signature.returnType.size()) {
	  printConstructor(base, className, classKind, constructor.argTypes,
			  signature);
	}
      }
    }
  ...
  if (exported.count(base) != 0) {
    printMethods(base, classKind, exported.at(base));
  }
  std::cout << "};\n";
}
